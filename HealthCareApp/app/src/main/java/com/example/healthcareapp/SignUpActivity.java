...
//            }
            ...
            writeNewUser(IDNum, name, password);
        }
    };

    private void writeNewUser(String IDNum, String name, String password) {
        try {
            String hashedPW = Utilities.hashPassword(password);
            User user = new User(IDNum, name, hashedPW);

            db.child("users").child(IDNum).setValue(user);
